...
import { BattleState, HAND_LIMIT, MAX_MANA_CAP, cloneState, opponentOf } from "./state";
...
export function endTurn(state: BattleState): CommandResult {
  ...
  if (incoming === "player") {
    ...
    if (next.playerHand.length < HAND_LIMIT && next.playerLibrary.length > 0) {
      const [drawn, ...library] = next.playerLibrary;
      next.playerHand = [...next.playerHand, drawn];
      next.playerLibrary = library;
    }
  } else {
    next.enemyMaxMana = Math.min(MAX_MANA_CAP, state.enemyMaxMana + 1);
    next.enemyMana = next.enemyMaxMana;
    if (next.enemyHand.length < HAND_LIMIT && next.enemyLibrary.length > 0) {
      const [drawn, ...library] = next.enemyLibrary;
      next.enemyHand = [...next.enemyHand, drawn];
      next.enemyLibrary = library;
    }
  }
